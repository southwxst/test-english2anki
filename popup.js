async function ankiInvoke(action, params = {}) {
  try {
    ...
    document.getElementById("status").innerText = "Failed to connect to Anki API";
    throw err;
  }
}
...
function loadSettings() {
  ...
  if (savedSettings) {
    ...
    // 値が存在する場合のみ設定を適用
    if (settings.deck && document.getElementById("deckSelect").querySelector(`option[value="${settings.deck}"]`)) {
      document.getElementById("deckSelect").value = settings.deck;
    }
    if (settings.model && document.getElementById("modelSelect").querySelector(`option[value="${settings.model}"]`)) {
      document.getElementById("modelSelect").value = settings.model;
      // モデルが変更されたのでフィールドを更新
      updateFields(settings.model);
    }
    if (settings.frontField && document.getElementById("frontField").querySelector(`option[value="${settings.frontField}"]`)) {
      document.getElementById("frontField").value = settings.frontField;
    }
    if (settings.backField && document.getElementById("backField").querySelector(`option[value="${settings.backField}"]`)) {
...
async function checkAnkiConnection() {
  try {
    ...
    if (result && result.result) {
      document.getElementById("status").innerText = "Connected to Anki (API version: " + result.result + ")";
      return true;
    } else {
      document.getElementById("status").innerText = "Anki API responded but with unexpected data";
      return false;
    }
  } catch (err) {
...
(async function () {
  try {
    ...
    document.getElementById("frontField").addEventListener("change", saveSettings);
    document.getElementById("backField").addEventListener("change", saveSettings);
...
async function updateFields(modelName) {
  try {
    ...
    if (savedSettings) {
      const settings = JSON.parse(savedSettings);
      if (settings.frontField && frontField.querySelector(`option[value="${settings.frontField}"]`)) {
        frontField.value = settings.frontField;
      }
      if (settings.backField && backField.querySelector(`option[value="${settings.backField}"]`)) {
...
document.getElementById("addCard").addEventListener("click", async () => {
  ...
  chrome.scripting.executeScript({
    ...
    try {
      ...
      document.getElementById("status").innerText = "Cards added successfully";
    } catch (e) {
      // Error already shown by ankiInvoke
    }
  });
...
function collectQuestions() {
  ...
  questions.forEach((question) => {
    if (question.querySelector('img[alt="Correct"]')) return;
    ...
    const feedbackEl = question.parentElement.querySelector(
      ".watupro-main-feedback.feedback-incorrect"
    );
    let feedbackText = feedbackEl ? feedbackEl.innerText.trim() : "";

    result.push({
      question: `Q: ${qText.trim()} Please correct this sentence.`,
